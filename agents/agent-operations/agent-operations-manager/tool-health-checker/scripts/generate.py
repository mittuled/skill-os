#!/usr/bin/env python3
"""
generate.py — Generates the output template for tool-health-checker

Renders the output template for tool-health-checker with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "tool-health-report-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "tool-health-checker"))
    return result
