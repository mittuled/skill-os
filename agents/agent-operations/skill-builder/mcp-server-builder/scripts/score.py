#!/usr/bin/env python3
"""
score.py — Scores an MCP server build against the quality rubric
...
RUBRIC = {
    "Endpoint Coverage": {"weight": 0.20, "description": "Percentage of API endpoints successfully mapped to MCP tools"},
    "Tool Granularity": {"weight": 0.20, "description": "Appropriateness of endpoint-to-tool mapping (not too coarse, not too fine)"},
    "Authentication Handling": {"weight": 0.20, "description": "Security of credential management using env vars, not hardcoded"},
    "Error Mapping Quality": {"weight": 0.20, "description": "MCP-compatible error responses for all HTTP error codes"},
    "Server Validity": {"weight": 0.20, "description": "Syntax correctness, no duplicate tools, transport initializes cleanly"},
}
