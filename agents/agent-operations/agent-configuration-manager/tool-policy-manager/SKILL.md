...
- [`tool-health-checker`](../../../agent-operations/agent-operations-manager/tool-health-checker/SKILL.md) — After policy changes, verify the newly added or modified tool is reachable and healthy.
- [`mcp-server-builder`](../../../agent-operations/skill-builder/mcp-server-builder/SKILL.md) — When adding an MCP-based tool, the server must be built before the policy entry is meaningful.
