#!/usr/bin/env python3
"""
generate.py — Generates the output template for option-pool-design

Renders the output template for option-pool-design with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "compute-allocation-strategy-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "option-pool-design"))
    return result
