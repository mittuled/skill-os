#!/usr/bin/env python3
"""
generate.py — Generates the output template for mcp-server-builder

Renders the output template for mcp-server-builder with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "mcp-build-report-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "mcp-server-builder"))
    return result
