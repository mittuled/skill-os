...
- [`tool-policy-manager`](../../agent-configuration-manager/tool-policy-manager/SKILL.md) — Register the newly built MCP server in allowed-tools.yaml after scaffolding.
- [`tool-health-checker`](../../agent-operations-manager/tool-health-checker/SKILL.md) — Verify the MCP server is healthy and reachable after deployment.
