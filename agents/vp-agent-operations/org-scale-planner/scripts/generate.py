#!/usr/bin/env python3
"""
generate.py — Generates the output template for org-scale-planner

Renders the output template for org-scale-planner with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "fleet-scale-plan-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "org-scale-planner"))
    return result
