#!/usr/bin/env python3
"""
generate.py — Generates the output template for customer-community-health-reviewer

Renders the output template for customer-community-health-reviewer with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "community-health-report-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "customer-community-health-reviewer"))
    return result
