#!/usr/bin/env python3
"""
generate.py — Generates the output template for customer-reference-programme-manager

Renders the output template for customer-reference-programme-manager with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "reference-programme-report-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "customer-reference-programme-manager"))
    return result
