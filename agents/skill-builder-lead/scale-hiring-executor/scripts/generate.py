#!/usr/bin/env python3
"""
generate.py — Generates the output template for scale-hiring-executor

Renders the output template for scale-hiring-executor with provided or placeholder data.
...
TEMPLATE_PATH = Path(__file__).parent.parent / "assets" / "scale-build-report-template.md"
...
def render_template(template: str, data: dict) -> str:
    ...
    result = result.replace("[skill-slug that produced this]", data.get("skill", "scale-hiring-executor"))
    return result
